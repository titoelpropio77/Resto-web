...
class DNS2D {
    ...
    public function getBarcodeSVG($code, $type, $w = 3, $h = 3, $color = 'black') {
        ...
        $svg .= "\t" . '<desc>' . tstrr($this->barcode_array['code'], $repstr) . '</desc>' . "\n";
     ...
     * @public
     */
    public function getBarcodePNGPath($code, $type, $w = 5, $h = 5, $color = array(0, 0, 0)) {
        if (!$this->store_path) {
             // $this->setStorPath(app('config')->get("barcode.store_path"));  
        }
        //set barcode code and type
        ...
        if (function_exists('imagecreate')) {
            ...
            $imagick = false;
            $png = imagecreate($width, $height);
            $bgcol = imagecolorallocate($png, 250, 255, 255);
            imagecolortransparent($png, $bgcol);
            $fgcol = imagecolorallocate($png, $color[0], $color[1], $color[2]);
        } elseif (extension_loaded('imagick')) {
            $imagick = true;
        ...
        // $file_name= "prueba";
        $save_file = $this->checkfile("CodigoControl/imagenQr/" . $file_name . ".png");
